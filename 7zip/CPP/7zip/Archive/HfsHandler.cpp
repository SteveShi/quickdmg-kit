...
struct CItem
{
  ...
  */
  ...
  bool IsSymLink() const { return (FileMode & 0xF000) == 0xA000; }
  UString SymLinkTarget;
...
HRESULT CDatabase::LoadCatalog(const CFork &fork, const CObjectVector<CIdExtents> *overflowExtentsArray, IInStream *inStream, IArchiveOpenCallback *progress)
{
...

  {
    FOR_VECTOR (i, Items)
    {
      ...
      if (item.IsSymLink() && item.DataFork.Size > 0 && item.DataFork.Size < (1 << 16))
      {
        CByteBuffer buf;
        if (ReadFile(item.DataFork, buf, inStream) == S_OK)
        {
          AString s;
          s.SetFrom_CalcLen((const char *)(const Byte *)buf, (unsigned)item.DataFork.Size);
          ConvertUTF8ToUnicode(s, item.SymLinkTarget);
        }
      }
...
Z7_COM7F_IMF(CHandler::GetProperty(UInt32 index, PROPID propID, PROPVARIANT *value))
{
  ...
  switch (propID)
  {
    ...
    case kpidSymLink:
      if (ref.IsItem() && item.IsSymLink() && !item.SymLinkTarget.IsEmpty())
        prop = item.SymLinkTarget;
      break;
    /*
